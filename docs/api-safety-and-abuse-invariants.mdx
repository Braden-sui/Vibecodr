...
- **[INV-API-0001] No dev/ops actions over HTTP**
  ...
  - **How we enforce it**
    ...
    - No route in `workers/api/src/routes.ts` may call these; they are wired only via `npm/pnpm` scripts.
...
- **[INV-API-0300] Network proxy and external fetches are allowlisted**
  ...
  - **How we enforce it**
    ...
    - Proxy is authenticated and only uses manifests for capsules owned by the caller; requests with foreign `capsuleId` values are rejected.
...
- **[INV-API-0502] Client must validate manifests before using capabilities**
  - **Goal**: UI should never trust unvalidated manifest JSON for capability flags or param shapes.
  - **Examples of what we are preventing**
    - A malformed or malicious manifest crashing a new UI surface that reads `capabilities`/`params` directly.
    - Rendering network/storage/worker badges from raw JSON without schema checks.
  - **How we enforce it**
    - Run `safeParseCapsuleManifest` (apps/web/lib/manifest.ts) on any manifest pulled in the browser before using its fields; fall back to `{ id, artifactId }` only if validation fails.
    - Server-side paths already run `validateManifest`/`requireCapsuleManifest`; keep parity in new client routes.
...
- **Scenario D: Proxy abuse**
  ...
  - **Mitigations**
    ...
    - Auth + per-owner scoping for `/api/proxy` so manifests cannot be borrowed across users.
...
## 10) Client Error Telemetry (Cloudflare Analytics Engine)

Client-side failures in Feed, Player, comments, and moderation must be observable in analytics, not just in browser consoles.

  - **Event shape**
    - All client error telemetry is sent via `trackClientError(code, props)` in `apps/web/lib/analytics.ts`.
  - This emits a `client_error` analytics engine datapoint with:
    - `code`: an `E-VIBECODR-####` string.
    - `type: "client_error"`.
    - Additional fields in `props` describing the area and context.

- **Key client error codes (frontend)**
  - **Feed / Home**
    - `E-VIBECODR-0507` — Feed error JSON parse failed.
      - `area: "feed"`, `stage: "error_json_parse"`, plus `status`, `mode`, `tagCount`.
    - `E-VIBECODR-0508` — Feed network error; request failed before response.
      - `area: "feed"`, `stage: "network"`, plus `mode`, `tagCount`.
  - **FeedCard / sharing / moderation**
    - `E-VIBECODR-0206` — Feed manifest preload failed (runtime manifest fetch).
      - `area: "feed.manifestPreload"`, plus `artifactId`, `postId`.
    - `E-VIBECODR-0107` — Feed moderation action error body JSON parse failed.
      - `area: "feed.moderationAction"`, plus `action`, `postId`, `status`.
    - `E-VIBECODR-0502` — Feed share via `navigator.share` failed or was cancelled.
      - `area: "feed.share"`, plus `postId`.
    - `E-VIBECODR-0503` — Feed clipboard write for share link failed.
      - `area: "feed.shareClipboard"`, plus `postId`.
  - **Player / runs / moderation / sharing**
    - `E-VIBECODR-0515` — Player `runsApi.appendLogs` call failed.
      - `area: "player.appendLogs"`, plus `runId`, `capsuleId`, `postId`.
    - `E-VIBECODR-0516` — Player `runsApi.complete` call failed.
      - `area: "player.completeRun"`, plus `runId`, `capsuleId`, `postId`, `status`.
    - `E-VIBECODR-0509` — Player moderation status JSON parse failed.
      - `area: "player.moderationStatus"`, `stage: "json_parse"`, plus `postId`, `status`.
    - `E-VIBECODR-0510` — Player moderation status fetch failed.
      - `area: "player.moderationStatus"`, `stage: "fetch"`, plus `postId`.
    - `E-VIBECODR-0511` — Player share via `navigator.share` failed or was cancelled.
      - `area: "player.share"`, plus `postId`.
    - `E-VIBECODR-0512` — Player clipboard write for share link failed.
      - `area: "player.shareClipboard"`, plus `postId`.
  - **Comments & reporting**
    - `E-VIBECODR-0504` — Comment create error JSON parse failed.
      - `area: "comments.create"`, plus `postId`, `status`.
    - `E-VIBECODR-0505` — Comment delete error JSON parse failed.
      - `area: "comments.delete"`, plus `postId`, `status`.
    - `E-VIBECODR-0506` — Report submit error JSON parse failed.
      - `area: "report.submit"`, plus `targetType`, `targetId`, `status`.
    - `E-VIBECODR-0514` — Admin moderation resolve error JSON parse failed.
      - `area: "admin.moderationResolve"`, plus `reportId`, `action`, `status`.
    - `E-VIBECODR-0513` — Flagged-post quarantine error JSON parse failed.
      - `area: "moderation.flagged.quarantine"`, plus `postId`, `status`.

- **Server-side vs client-side observability**
  - Worker and Next.js routes log with `console.error` and structured `E-*` codes to Workers Analytics and platform logs.
  - Client-only paths (React components, browser-only flows) **must** use `trackClientError` in addition to local `console.error` so we can:
    - Measure error rate and cohort impact.
    - Correlate with usage events like `feed_results_loaded`, `player_run_started`, or `composer_*`.

- **Querying and dashboards**
    - In the Cloudflare Analytics Engine:
    - Filter events where `event = "client_error"` and group by `code` and `area`.
    - Join with `feed_results_*` and `player_*` events on session or user to see how often errors affect core journeys.
  - For Workers Analytics:
    - Use the same `E-VIBECODR-####` codes to correlate backend failures (e.g., proxy, manifest, quotas) with client-side error events.

## 11) Implementation map (where these invariants live)

- **Public read endpoints rate limits:** enforced via Cloudflare WAF for `/api/*`; heavy paths also add per-request caps (see `/api/proxy` per-user/per-IP/per-capsule+host throttles in `workers/api/src/handlers/proxy.ts`).
- **Expensive actions quota-limited:** bundle/storage/run limits defined in `workers/api/src/storage/quotas.ts` and enforced across publish/import (`workers/api/src/handlers/capsules.ts`, `workers/api/src/handlers/import.ts`), runs (`workers/api/src/handlers/runs.ts`), and artifact compile flows.
- **Proxy not a free scraper:** strict host allowlist merged with `ALLOWLIST_HOSTS`, blocked localhost/IP ranges, and plan-aware disable for free tier in `workers/api/src/handlers/proxy.ts`; responses surface 403/429 with structured codes.
- **Safety hooks on user code:** publish/import paths call `enforceSafetyForFiles` (safetyClient) and propagate structured `PublishCapsuleError` bodies to the SPA (`workers/api/src/handlers/capsules.ts`, `workers/api/src/handlers/import.ts`).
- **Edge WAF status:** Account is on Free zone; Cloudflare WAF rate limits not available. Interim: in-Worker per-IP rate limits on public reads (capsules, artifacts, embeds/oEmbed/OG image) and proxy, strict `ALLOWLIST_HOSTS`, proxy disabled for free tier. TODO when upgraded: add WAF rate-limit/challenge rules for `/api/proxy` and public reads.
