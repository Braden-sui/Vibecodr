...
- [x] Secrets configured and documented (Clerk, PostHog, etc.) with rotation expectations.
...
- [ ] Session handling and token propagation between Pages and Workers documented and validated end-to-end.
...
- [x] Badges on cards: runner type, net access, params count.
...
- [ ] Console proxy + sampled analytics wired to PostHog/Analytics Engine.
- [ ] Restart without full reload where possible (warm restarts).
- [ ] Preview-to-Player handoff preserves param state and logs.

## 6) Manifest, Runner, and Runtime Artifacts (Phase 2)

- [x] Manifest schema defined (params, capabilities: `net[]`, `storage`, `workers`, live, concurrency).
...
- [ ] Technical budgets enforced (client-static boot <= 1s P95; CPU wall-time and memory heuristics documented and enforced).
...
- [x] Unified `VibesComposer` on the feed for status posts, image attachments, GitHub imports, ZIP uploads, and inline code.
...
- [x] PostHog initialized on web; Workers Analytics Engine bound in API.
...
- [x] Workers Analytics Engine + PostHog for observability.
...
- [ ] CPU/memory/time budgets and kill switch in Player/UI for abusive runtimes.
