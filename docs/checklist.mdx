...
- [x] Console proxy + sampled analytics wired to PostHog/Analytics Engine.
...
- [ ] Refactor `/import/zip` into an analyze + prebuild helper (manifest + warnings only) so `/capsules/publish` remains the single write path for capsule creation.
