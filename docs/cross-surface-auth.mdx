...
2. **Studio entry** mounts `EnsureUserSynced`, which uses Clerk client hooks to call `ensureUserSynced({ user, token })`. That helper POSTs directly to `${WORKER_API_BASE}/users/sync` with `Authorization: Bearer <worker-token>` and the Clerk-normalized profile.
...
sequenceDiagram
  ...
  Browser->>WorkerAPI: POST /users/sync (Clerk cookies, via token from useAuth.getToken("workers"))
  WorkerAPI->>Clerk: Verify JWT via JWKS
