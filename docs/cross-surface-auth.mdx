...
2. **Studio entry** mounts `EnsureUserSynced`, which uses Clerk client hooks to call `ensureUserSynced({ user, token })`. That helper POSTs directly to `${getWorkerApiBase()}/users/sync` with the normalized **user** payload (id, handle, name, avatarUrl, bio). Profile cosmetics are edited via `/profile`; ownership stays on `users`.
...
sequenceDiagram
  ...
  Browser->>WorkerAPI: POST /users/sync (Bearer token + user payload)
  WorkerAPI->>Clerk: Verify JWT via JWKS
