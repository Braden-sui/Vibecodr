...
export const syncUser: Handler = requireUser(async (req, env, _ctx, _params, authedUserId) => {
  ...
  const { id, handle, name, avatarUrl, bio, plan, email } = base.data;
  if (id !== authedUserId) {
  ...
  try {
    ...
    await env.DB.prepare(
      `INSERT INTO users (id, handle, name, avatar_url, bio, email, plan)
       VALUES (?, ?, ?, ?, ?, ?, COALESCE(?, 'free'))`
    ).bind(id, handle, name ?? null, avatarUrl ?? null, bio ?? null, email ?? null, plan ?? null).run();
